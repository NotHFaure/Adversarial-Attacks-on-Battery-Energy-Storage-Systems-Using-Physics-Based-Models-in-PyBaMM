...
# Define the time range for the simulation (from 0 to 1000 seconds, with 500 points)
...
# Adversarial SEI resistivity attack function
def adversarial_SEI_resistivity(t, resistivity_spike, for_plot=False):
    ...
    # Apply SEI resistivity spike during the attack window
...
def objective(params):
    ...
    param_attack.update({
        ...
        "SEI resistivity [Ohm.m]": adversarial_SEI_resistivity(pybamm.t, resistivity_spike)
    }, check_already_exists=False)
   
    # Solve the model with adversarial attack
    sim = pybamm.Simulation(model, parameter_values=param_attack)
...
def plot_results(optimal_params, t_eval, voltage_normal, current_normal):
    ...
    param_attack.update({
        ...
        "SEI resistivity [Ohm.m]": adversarial_SEI_resistivity(pybamm.t, optimal_params[3])
    })
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(t_eval, adversarial_SEI_resistivity(t_eval, optimal_params[3], for_plot=True), label="Adversarial SEI Resistivity", color='red')
    plt.axvline(x=attack_start_time, color='green', linestyle='--', label="Attack Start")
    plt.axvline(x=attack_end_time, color='orange', linestyle='--', label="Attack End")
    plt.xlabel("Time [s]")
...
initial_guess = [0.005, 0.02, 10, 1e-3]  # Add SEI resistivity spike to the list
bounds = [(0, 0.1), (0, 0.05), (0, 15), (0, 1e-2)]  # Bounds for SEI resistivity spike
