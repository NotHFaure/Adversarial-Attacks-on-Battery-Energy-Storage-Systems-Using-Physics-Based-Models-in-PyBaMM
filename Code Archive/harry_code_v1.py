...
def objective(params):
    perturbation_amplitude, perturbation_increase, temperature_spike = params
...
def plot_results(optimal_params, t_eval, voltage_normal, current_normal):
    # Solve with optimal parameters for current and temperature attacks
    param.update({
        "Current function [A]": adversarial_current(pybamm.t, optimal_params[0], optimal_params[1]),
        "Ambient temperature [K]": adversarial_temperature(pybamm.t, optimal_params[2])  # Add temperature perturbation
    })
    ...
    plt.plot(t_eval, voltage_interp, label="Combined Adversarial Attack (Current + Temperature)", color='red')
    ...
    plt.title("Terminal Voltage: Combined Current and Temperature Attack")
    plt.legend()
    plt.grid(True)
...
initial_guess = [0.005, 0.02, 10]  # Add temperature spike to the list
bounds = [(0, 0.1), (0, 0.05), (0, 15)]  # Add bounds for temperature spike (from 0 to 15 K)
